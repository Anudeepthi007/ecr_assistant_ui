...
@tool("build_correlation_graph", "Link every retrieved artefact to the ECR with a typed edge.")
def build_correlation_graph(state: dict[str, Any]) -> dict[str, Any]:
    ...
    for comment in comments:
        ...
        for reference in comment.get("references") or []:
            add_edge(cid, reference, "REFERENCES", "Comment explicitly references this artefact", 0.5)
    ...
    for item in evidence:
        ...
        add_edge(
            ...
            eid,
            "EVIDENCED_BY",
            f"{item.get('evidence_type', 'Artefact')} with outcome {item.get('outcome', 'UNKNOWN')}",
            0.7,
        )
...
def correlation_step(state: dict[str, Any]) -> dict[str, Any]:
    ...
    # Cross-source corroboration: artefacts named by more than one source are the
    ...
    corroborated = sorted(
        (
            {"artefact": artefact, "sources": sorted(sources), "source_count": len(sources)}
            for artefact, sources in mention_count.items()
            if len(sources) > 1
        ),
    ...
    reasoning = (
        f"Correlated {len(graph['nodes'])} artefact(s) and {len(graph['edges'])} typed link(s) "
        f"for {ecr_id} across {sum(1 for v in coverage.values() if v)} source(s). "
        f"{len(corroborated)} artefact(s) are corroborated by more than one source."
    )
